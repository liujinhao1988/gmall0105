...
public class PaymentController {
    ...
    public  String aliPayCallBackReturn( HttpServletRequest request, ModelMap modelMap){
        ...
        //通过支付宝的paramsMap进行签名验证，2.0版本的接口将paramsMap参数去掉了，导致同步秦球没法验证签名
        if(StringUtils.isNotBlank(sign)){
            ...
            paymentInfo.setAlipayTradeNo(trade_no);//支付宝的交易凭证好
            paymentInfo.setCallbackContent(call_back_content);//回调请求字符锤昂
    ...
    public  String alipay(String outTradeNo, BigDecimal totalAmount, HttpServletRequest request, ModelMap modelMap){
        ...
        //map.put("total_amount",totalAmount);//实际
        map.put("subject","ljhljhljh");
        ...
        paymentInfo.setSubject("商品");
        paymentInfo.setTotalAmount(totalAmount);
        paymentService.savePaymentInfo(paymentInfo);


        //向消息中间件发送一个检查支付状态的延迟消息队列（由支付服务消费）
